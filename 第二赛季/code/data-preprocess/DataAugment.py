...
def show_pic(new_pic_name,img,labels_old,bboxes=None):
    '''
    输入:
        img:图像array
    ...
    new_txt_name = new_pic_name[0:-4]+".txt"
    with open("Labels/"+new_txt_name,"w")as fw:
        for i in range(len(bboxes)):
            ...
            x_max = bbox[2]
            y_max = bbox[3]
            fw.write(new_pic_name[0:-4]+" "+labels_old[i]+" "+str(x_min)+" "+str(x_max)+" "+str(y_min)+" "+str(y_max)+"\n")
...
class DataAugmentForObjectDetection():
    ...
    def dataAugment(self, img, bboxes):
        ...
        print('------')
        while change_num < 1:
            my_choice = random.random()  
            if my_choice < 0.1 and my_choice>=0:        #裁剪
                print('裁剪')
                change_num += 1
                img, bboxes = self._crop_img_bboxes(img, bboxes)
            
            elif my_choice >=0.1 and my_choice<0.3:    #旋转
                print('旋转')
                change_num += 1
            ...
            elif my_choice >=0.3 and my_choice<0.5:        #平移
                print('平移')
                change_num += 1
                img, bboxes = self._shift_pic_bboxes(img, bboxes)
            
            elif my_choice>=0.5 and my_choice<0.6: #改变亮度
                print('亮度')
                change_num += 1
                img = self._changeLight(img)
            
            elif my_choice >=0.6 and my_choice<0.7:    #加噪声
                print('加噪声')
                change_num += 1
                img = self._addNoise(img)

            elif my_choice >=0.7 and my_choice<0.8:  #cutout
                print('cutout')
                change_num += 1
                img = self._cutout(img, bboxes, length=self.cut_out_length, n_holes=self.cut_out_holes, threshold=self.cut_out_threshold)

            elif my_choice >=0.8 and my_choice<1:    #翻转
                print('翻转')
                change_num += 1
    ...
    for parent, _, files in os.walk(source_pic_root_path):
        for file in files:
            ...
            while cnt < need_aug_num:
                ...
                cv2.imwrite("new_dansheng_pic/"+new_pic_name,auged_img)
                cnt += 1

                show_pic(new_pic_name,auged_img,labels_old,auged_bboxes)  # 强化后的图
